...
$showNotice = !empty($_SESSION['show_notice']) && $_SESSION['show_notice'] === true;

// 권한별로 지정된 공지 이후 이동 URL (없으면 기본값 세팅)
if (!empty($_SESSION['after_notice_url'])) {
    $afterNoticeUrl = (string)$_SESSION['after_notice_url'];
} else {
    // 혹시 세션에 없다면 user_type 기준으로 백업 경로 설정
    $userType = isset($_SESSION['user_type']) ? (string)$_SESSION['user_type'] : 'user';
